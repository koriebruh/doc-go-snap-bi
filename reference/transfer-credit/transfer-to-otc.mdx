...
TransferToOTCTransferStatusRequest is the request body for API Transfer To OTC - Transfer Status (Service Code 45). It follows the same shape as [`TransactionStatusInquiryBankRequest`](/reference/transfer-credit/transaction-status), plus CustomerNumber and a Mandatory (not Optional) Amount. ServiceCode and CustomerNumber are Mandatory.
...
TransferToOTCTransferStatusResponse is the response body for API Transfer To OTC - Transfer Status — field-identical to [`TransactionStatusInquiryBankResponse`](/reference/transfer-credit/transaction-status), modeled as its own type since each SNAP service code gets a distinct type even when shapes match.
