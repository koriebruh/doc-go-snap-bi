...
signing mode (see [Signing](/concepts/signing)).
...
Concurrent callers during a refresh share one in-flight HTTP request
rather than each firing their own — a mutex-guarded single-flight. The
refresh itself runs on a context detached from any individual caller, so
one caller's cancellation can't poison the result every other concurrent
caller receives.
...
<CardGroup cols={2}>
  <Card title="Headers & Signing" icon="key" href="/concepts/headers">
    Using the token you got here to build a signed transaction request.
  </Card>
  <Card title="Registration" icon="id-card" href="/reference/registration">
    `GetOAuthURL` and `AccountBinding` — the B2B2C onboarding flow.
  </Card>
</CardGroup>
