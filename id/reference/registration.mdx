...
Setiap fungsi di bawah punya bentuk pemanggilan yang sama: isi `hb` (semua field kecuali `Body` — fungsinya sendiri yang marshal request dan mengatur `Body`), panggil, lalu cek `err`. Lihat [Core Conventions](/id/concepts/conventions) untuk pola lengkapnya.
