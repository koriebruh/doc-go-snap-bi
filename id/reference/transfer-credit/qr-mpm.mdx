...
<Accordion title="Field ApplyOTTUserResource">
...
<Accordion title="Field MPMMerchantInfo">
...
<Info>Hanya masuk (inbound) &mdash; package ini tidak memanggil endpoint untuk ini. Partner/switcher yang POST ke callback URL **Anda**; unmarshal body ke `QRMPMPaymentNotificationRequest` setelah memverifikasinya dengan `snap.ServerVerifier` (lihat [Verifying inbound requests](/id/concepts/webhooks)), lalu balas dengan bentuk di bawah.</Info>
...
Field dasar yang sama dengan [`TransactionStatusInquiryBankRequest`](/id/reference/transfer-credit/transaction-status), ditambah `MerchantID`, `SubMerchantID`, dan `ExternalStoreID`. `ServiceCode` satu-satunya field wajib.
...
Field yang sama dengan [`TransactionStatusInquiryBankResponse`](/id/reference/transfer-credit/transaction-status), ditambah `PaidTime` dan `TerminalID`.
...
`CancelTime` Kondisional (dimodelkan Opsional, sesuai penanganan standar package ini — lihat [Core Conventions](/id/concepts/conventions)); `TransactionDate` Opsional.
