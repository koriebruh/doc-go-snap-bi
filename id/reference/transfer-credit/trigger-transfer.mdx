...
<Info>Hanya masuk (inbound) &mdash; package ini tidak memanggil endpoint untuk ini. Partner/switcher yang mengirim POST ke callback URL **Anda**; unmarshal body-nya ke `InterbankBulkTransferNotificationRequest` setelah diverifikasi dengan `snap.ServerVerifier` (lihat [Verifying inbound requests](/id/concepts/webhooks)), lalu balas dengan bentuk di bawah.</Info>
...
<Info>Hanya masuk (inbound) &mdash; package ini tidak memanggil endpoint untuk ini. Partner/switcher yang mengirim POST ke callback URL **Anda**; unmarshal body-nya ke `RTGSNotificationRequest` setelah diverifikasi dengan `snap.ServerVerifier` (lihat [Verifying inbound requests](/id/concepts/webhooks)), lalu balas dengan bentuk di bawah.</Info>
...
<Info>Hanya masuk (inbound) &mdash; package ini tidak memanggil endpoint untuk ini. Partner/switcher yang mengirim POST ke callback URL **Anda**; unmarshal body-nya ke `SKNBINotificationRequest` setelah diverifikasi dengan `snap.ServerVerifier` (lihat [Verifying inbound requests](/id/concepts/webhooks)), lalu balas dengan bentuk di bawah.</Info>
